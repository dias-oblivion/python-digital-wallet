...
percorre a AST de cada módulo e falha se alguma camada importar o que não deve. Durante a
...
O contrato fala só de entidades ([`ports/repositories.py`](src/wallet/ports/repositories.py)):
...
([`services/auth.py`](src/wallet/services/auth.py)):
...
O **composition root** ([`api/wiring.py`](src/wallet/api/wiring.py)) é o único lugar que conhece a
...
[`db/connection.py`](src/wallet/db/connection.py): mypy vê os parâmetros de tipo, o interpretador vê
a classe crua.
